class Array
    ...
    def my_each_with_block(&prc) #block convert to prc
        ...
        while i < self.length
            ...
            prc.call(ele)arr = Array.new(self.length) {Array.new(num.length)}
            i += 1
        end
...
#block_given? is a built in ruby boolean method to check if a block was given to the method
    ...
    #block is the only thing in Ruby that is not an OBJECT
    #method can be consider not an object as well
