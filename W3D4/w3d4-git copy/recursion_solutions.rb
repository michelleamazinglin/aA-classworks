...
class Array
  def deep_dup
    # every stack frame/recursive call has it's own scope
    new_array = []
    # self = [1, [2, 3]] | [2, 3]
    # base case? - when every element in the array is not a subarray
    self.each do |el|
      if el.is_a? Array # when el is subarray
        new_array << el.deep_dup # [2, 3].deep_dup = [2, 3] => [1, [2, 3]]
      else # when el is just an element
        new_array << el # [1] | [2, 3]
      end
