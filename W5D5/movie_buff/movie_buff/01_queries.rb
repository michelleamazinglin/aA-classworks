...
def harrison_ford
  ...
  Movie.select(:id, :title)
       .joins(:actors) #actors here is a association method name
       .where(actors: {name: 'Harrison Ford'}) #actors here is a table name
       .where("ord != 1")

        # AND lead_actor != 'Harrion Ford'
end
...
def biggest_cast
  ...
  Movie
    .select(:id, :title)
    .joins(:actors)
    .group('movies.id') #<- is this referring to ID in movies table
